...
function init(callback) { // @param Function(= void): callback
    // --- setup behavior ---
    this.storage = uu.node.add(doc.createElement("script"), doc.head); // <script>
...
function nth(index) { // @param Number:
    ...
    return (this.storage.getAttribute(_INDEX) || "", 1).split("\t")[index] || "";
}
...
function get(key) { // @param String:
    ...
    this.storage.load(_STORE_NAME);
    return this.storage.getAttribute(key, 1) || "";
}
...
function getAll() { // @return Hash: { key: "value", ... }
    ...
    var rv = {}, key, i = -1,
        indexes = (this.storage.getAttribute(_INDEX) || "", 1).split("\t");

    while ( (key = indexes[++i]) ) {
        rv[key] = this.storage.getAttribute(key, 1) || "";
    }
    return rv;
}
...
function set(key,     // @param String:
    ...
    var index = this.storage.getAttribute(_INDEX, 1);
    ...
    // verify
    return this.storage.getAttribute(key, 1) === value;
}
...
/*
    ...
    while ( (key = indexes[++i]) ) {
        used += (this.storage.getAttribute(key, 1) || "").length;
    }
    return { used: used, max: _DISK_SPACE };
 */
    this.storage.load(_STORE_NAME);
    return { used: this.storage.remainingSpace(), max: _DISK_SPACE };
}

// IEStorage.pairs
function pairs() { // @return Number: pairs
    this.storage.load(_STORE_NAME);
    return (this.storage.getAttribute(_INDEX) || "", 1).split("\t").length;
}
...
function clear() {
    ...
    var indexes = (this.storage.getAttribute(_INDEX) || "", 1).split("\t"),
        key, i = -1;
