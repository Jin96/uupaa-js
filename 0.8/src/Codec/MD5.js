...
uu.Class.singleton("MD5", {
    init:       init,
    encode:     encode      // encode(str:String, unicode:Boolean = false):String
});

function init() {
    // self test
    var test = [
//      _md5ascii(""),              "d41d8cd98f00b204e9800998ecf8427e",
        _md5ascii("hoge"),          "ea703e7aa1efda0064eaa507d9e8ab7e",
//      _md5ascii("ascii"),         "5b7f33be48f19c25e1af2f96cffc569f",
//      _md5ascii("user-password"), "9a3729201fdd376c76ded01f986481b1"
    ], i = 0, iz = test.length;

    for (; i < iz; i += 2) {
        if (test[i] !== test[i + 1]) {
            throw Error("FAIL_MD5_SELF_TEST");
        }
    }
}

// encode - encode md5
function encode(str,       // @param String:
                unicode) { // @param Boolean(= false): false is ascii
                           // @return String:
    return (unicode ? _md5ucs2 : _md5ascii)(str);
}

// inner - encode ascii to md5
...
// inner -
function _md5(bary,   // @param ByteArray:
              size) { // @param Number: ByteArray length (string.length)
